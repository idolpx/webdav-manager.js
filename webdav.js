...
const WebDAVNavigator = async function (url, options) {
    ...
    class TextEditor {
        ...
        bind(textEl, statusEl, saveBtn, closeBtn, autosaveCheckbox) {
            if (!textEl)
                return;
            // Initialize content
        ...
        bindMarkdownPreview(textEl, previewEl) {
            if (!textEl || !previewEl || !this.deps.markdownToHTML)
                return;
            textEl.oninput = () => {
            ...
            textEl.onscroll = (e) => {
                ...
                const target = e.target === previewEl ? textEl : previewEl;
    ...
    // Classes ///////
    var dav = { 'headers': {} }, wopi = { 'discovery_url': null, 'mimes': {}, 'extensions': {} }, browser = {
        file: {},
        paste_selection: [],
        paste_action: null,
        sort_order: 'name',
    ...
    browser.editTextFile = (file) => {
        // Fetch file content
        req('GET', file.url).then((r) => r.text()).then((content) => {
            ...
            const autosaveCheckbox = dlg.querySelector('.editor-toolbar .autosave');
    ...
    browser.createPasteSelection = (action) => {
        var l = Array.from(document.querySelectorAll('input[name=delete]:checked'));
        if (!l.length) {
    ...
    browser.downloadSelectedFiles = async () => {
        var items = Array.from(document.querySelectorAll('tbody input[type=checkbox]:checked'));
    ...
    const reqOrError = (method, url, body) => {
        return req(method, url, body).then(r => reqHandler(r)).catch(e => {
            console.error(e);
            alert(e);
            browser.reload();
            throw e;
    ...
    const openDialog = (html, ok_btn = true) => {
        ...
        if (a = $('dialog form input, dialog form textarea'))
            a.focus();
    };
    const closeDialog = (e) => {
    ...
    window.addEventListener('drop', (e) => {
        ...
        document.body.classList.remove('dragging');
        dragcounter = 0;
        var files = Array.from(e.dataTransfer.items).map((item) => item.getAsFile());
